...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof smaxabssorted, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( smaxabssorted.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the maximum absolute value of a sorted one-dimensional ndarray', function test( t ) {
	var x;
	var v;

	x = new Float32Array( [ -4.0, -2.0, 0.0, 1.0, 3.0, 5.0 ] );
	v = smaxabssorted( [ vector( x, 6, 1, 0 ) ] );
	t.strictEqual( v, 5.0, 'returns expected value' );

	x = new Float32Array( [ -4.0, -5.0 ] );
	v = smaxabssorted( [ vector( x, 2, 1, 0 ) ] );
	t.strictEqual( v, 5.0, 'returns expected value' );

	x = new Float32Array( [ -0.0, 0.0, -0.0 ] );
	v = smaxabssorted( [ vector( x, 3, 1, 0 ) ] );
	t.strictEqual( isPositiveZerof( v ), true, 'returns expected value' );

	x = new Float32Array( [ NaN ] );
	v = smaxabssorted( [ vector( x, 1, 1, 0 ) ] );
...
tape( 'if provided an empty ndarray, the function returns `NaN`', function test( t ) {
	...
	v = smaxabssorted( [ vector( x, 0, 1, 0 ) ] );
	t.strictEqual( isnanf( v ), true, 'returns expected value' );
...
tape( 'the function supports one-dimensional ndarrays having non-unit strides', function test( t ) {
	...
	v = smaxabssorted( [ vector( x, 4, 2, 0 ) ] );
...
tape( 'the function supports one-dimensional ndarrays having negative strides', function test( t ) {
	...
	v = smaxabssorted( [ vector( x, 4, -2, 6 ) ] );
...
tape( 'the function supports one-dimensional ndarrays having non-zero offsets', function test( t ) {
	...
	v = smaxabssorted( [ vector( x, 4, 2, 1 ) ] );
	t.strictEqual( v, 4.0, 'returns expected value' );
